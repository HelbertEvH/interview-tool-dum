...
if not st.session_state.setup_complete: 
    ...
    with col1:
        st.session_state["level"] = st.radio(
            "Choose Level",
            key="visibilty",
            options=["Junor", "Mid-Level", "Senior"],
        )
...
if st.session_state.setup_complete and not st.session_state.feedback_shown and not st.session_state.chat_complete:
    ...
    if "openai_model" not in st.session_state:
        st.session_state["openai_model"] = "gpt-5.4-nano"

    if not st.session_state.messages:
        st.session_state.messages = [{"role":"system", "content": f"You are an HR executive that interviews an interviewee called {st.session_state["name"]} with expirience {st.session_state["experience"] } and skills {st.session_state["skills"]}. You should interview him for the position {st.session_state["level"]} {st.session_state["position"]} at the company {st.session_state["company"]}"}]
